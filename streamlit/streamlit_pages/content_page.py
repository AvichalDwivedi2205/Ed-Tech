...
from backend.agents import ContentCreatorAgent, ContextManager
import matplotlib
matplotlib.use('Agg')  # Use non-interactive backend for Streamlit

def render():
    """Render the content creator page"""
    ...
    st.markdown("---")
        ...
        if st.button("🗑️ Delete All Generated Content", help="Delete all generated content files"):
            ...
            generated_content_dir = os.path.join(backend_dir, "generated_content")
            ...
            if st.session_state.generated_content and st.session_state.current_subtopic == next_subtopic_id:
                ...
                if st.session_state.get("generated_graphs"):
                    ...
                    for i, graph in enumerate(st.session_state.generated_graphs, 1):
                        ...
                        if graph_code:
                            # Aggressively clean the code - remove all instructions and markdown
                            clean_code = graph_code
                            
                            # Remove markdown code blocks
                            clean_code = re.sub(r'```python\s*', '', clean_code)
                            clean_code = re.sub(r'```json\s*', '', clean_code)
                            clean_code = re.sub(r'```\s*$', '', clean_code)
                            
                            # Remove instruction lines (lines containing "Do NOT", "MUST", "CRITICAL", etc.)
                            ...
                            for line in lines:
                                ...
                                if any(phrase in line_lower for phrase in ['do not include', 'do not', 'must include', 'critical', 'requirements:', 'output format', 'return only']):
                                    skip_next = True
                                    continue
                            ...
                            clean_code = re.sub(r'plt\.show\(\)\s*', '', clean_code, flags=re.MULTILINE)
                            
                            # Remove comments that are instructions
                            clean_code = re.sub(r'#\s*(Do NOT|MUST|CRITICAL|Output|Return).*?\n', '\n', clean_code, flags=re.IGNORECASE)
                            
                            clean_code = clean_code.strip()
                            
                            # Ensure imports are present at the start
                            ...
                            if 'import numpy' not in clean_code:
                                ...
                                clean_code = clean_code.replace('import matplotlib.pyplot as plt', 'import matplotlib.pyplot as plt\nimport numpy as np')
                                if 'import numpy' not in clean_code:
                            ...
                            if 'np.random.seed' not in clean_code and 'random.seed' not in clean_code:
                                ...
                                if 'import numpy as np' in clean_code:
                                    clean_code = clean_code.replace('import numpy as np', 'import numpy as np\nnp.random.seed(42)')
                                else:
                                    clean_code = 'import numpy as np\nnp.random.seed(42)\n' + clean_code
                            
                            # Ensure we have a figure before tight_layout
                            if 'plt.tight_layout()' in clean_code and 'plt.figure' not in clean_code and 'fig' not in clean_code:
                                # Add figure creation if missing
                                if 'plt.subplots' not in clean_code:
                                    clean_code = clean_code.replace('plt.tight_layout()', 'plt.figure(figsize=(10, 6))\nplt.tight_layout()')
                            ...
                            try:
                                ...
                                if fig and len(fig.get_axes()) > 0:
                                    ...
                                    st.warning("⚠️ Could not generate graph. The code may be incomplete.")
        ...
        if completed_ids:
            ...
            for subtopic_id in completed_ids:
                with st.expander(f"{subtopic_id} - View Full Content", expanded=False):
                    ...
                    import os
                    backend_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                    content_file = os.path.join(backend_dir, "generated_content", subtopic_id, "content.md")
